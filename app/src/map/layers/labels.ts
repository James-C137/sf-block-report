...
export function addLabelLayers(map: MLMap, fc: GeoJSON.FeatureCollection): void {
  ...
  /* the pin-label layer (pings.ts) may already be up — slot beneath it so
     the user's pins out-place the wayfinding in symbol collision */
  const beforePins = map.getLayer('pin-labels') ? 'pin-labels' : undefined;
  const layout = {
